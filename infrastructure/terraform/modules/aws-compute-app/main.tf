...
resource "aws_key_pair" "deployer" {
  ...
  # NOTA: Asegúrate de que esta sea la llave PÚBLICA correcta (la misma de los otros módulos)
...
# ----------------------------------------------------------------------------
  ingress {
    description = "Allow SSH strictly from QA Network (Bastion)"
    from_port   = 22
    to_port     = 22
    protocol    = "tcp"
  ...
  # TIP: También es buena práctica abrir esto a toda la red QA si el Gateway está allí.
  ingress {
    description = "Microservices Traffic from QA Gateway"
    from_port   = 3000
    to_port     = 4000
    protocol    = "tcp"
    cidr_blocks = ["10.0.0.0/16"] # Ampliado para evitar problemas de peering
  }
   
  # Allow HTTP (80) for Frontend
  # ESTO SE MANTIENE ABIERTO AL MUNDO SOLO SI ES UN NODO PÚBLICO.
  # SI ES PRIVADO, DEBERÍA SER SOLO DESDE EL LOAD BALANCER.
  # (Por ahora lo dejamos igual para no romper tu flujo actual si usas acceso directo)
  ingress {
